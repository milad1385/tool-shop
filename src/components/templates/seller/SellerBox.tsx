...
function SellerBox({ title, image, link, city, productCount }: ISeller) {
  return (
      ...
      >
        <div className="flex flex-col  absolute z-20 top-4 left-4 bg-white border-t-4 border-yellow-400 p-2 px-3 rounded-xl">
          <span className="font-IranMedium font-bold text-lg md:text-2xl">
            {formattedPrice(productCount)}
          </span>
          <span className="font-IranMedium text-xs text-gray-600 md:text-sm">{city}</span>
        </div>
